...
class EmulationManager {
  ...
  async clearViewportOverride (props) {
    const opts = {}
    if (props.width) opts.width = 0
    if (props.height) opts.height = 0
    if (props.deviceScaleFactor) opts.deviceScaleFactor = 0
    if (props.scale) opts.scale = 1
    if (props.mobile && this._emulatingMobile !== props.mobile) {
      opts.mobile = !props.mobile
      this._emulatingMobile = opts.mobile
    }
    await this._client.send('Emulation.setDeviceMetricsOverride', opts)
  }
