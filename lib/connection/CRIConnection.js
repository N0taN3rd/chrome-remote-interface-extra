...
class CRIConnection extends Chrome {
  ...
  /**
   * Enables the emitting of every msg received from the remote browser via {@link Events#CRIConnection#Verbose}
   */
  enableVerboseEmitting () {
    this._verboseEmitting = true
  }

  /**
   * Disables the emitting of every msg received from the remote browser
   */
  disableVerboseEmitting () {
    this._verboseEmitting = true
  }
  ...
  _handleMessage (object) {
    if (this._verboseEmitting) {
      this.emit(Events.CRIConnection.Verbose, object)
    }
