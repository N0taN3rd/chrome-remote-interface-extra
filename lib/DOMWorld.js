...
class DOMWorld {
  ...
  /**
   * Returns a JSHandle for this worlds window object
   ...
   */
  async window () {
    const executionContext = await this._contextPromise
  ...
  async evaluateWithCliAPI (pageFunction, ...args) {
    const context = await this.executionContext()
    return context.evaluateWithCliAPI(pageFunction, ...args)
  }
  ...
  async _waitForSelectorOrXPath (selectorOrXPath, isXPath, options = {}) {
    ...
    // https://github.com/GoogleChrome/puppeteer/pull/4373 @razorman8669
    /**
     * @param {string} selectorOrXPath
     * @param {boolean} isXPath
