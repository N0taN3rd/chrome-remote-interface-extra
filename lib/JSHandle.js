...
class JSHandle {
  ...
  toString () {
    if (this._remoteObject.objectId) {
      let type
      if (this._remoteObject.subtype === 'node') {
        type = `${this._remoteObject.className}-${
          this._remoteObject.description
        }`
      } else {
...
class ElementHandle extends JSHandle {
  ...
  hasChildNodes () {
    return this.executionContext().evaluate(elem => elem.hasChildNodes(), this)
  }
...
function intersectQuadWithViewport (quad, width, height) {
  const intersection = new Array(quad.length)
  for (let i = 0; i < quad.length; i++) {
    const point = quad[i]
    intersection[i] = {
