...
class Page extends EventEmitter {
  ...
  async setCookies (...cookies) {
    ...
    const startsWithHTTP = pageURL.startsWith('http')
    for (let i = 0; i < cookies.length; i++) {
      const cookie = ensureCookie(cookies[i], pageURL, startsWithHTTP)
      await this._networkManager.deleteCookie(cookie)
      await this._networkManager.setCookies(cookie)
    }
  }
  /**
 ...
 * @return {CDPCookie}
 */
function ensureCookie (cookie, url, setURLIfMissing) {
  let ensuredCookie
  ...
  if (!ensuredCookie.url && setURLIfMissing) ensuredCookie.url = url
