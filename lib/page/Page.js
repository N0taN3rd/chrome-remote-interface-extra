...
const FileChooser = require('./FileChooser')
...
class Page extends EventEmitter {
  ...
  static async create (client, optionals = {}) {
    ...
    await Promise.all([
      ...
      page.networkManager.initialize(),
  ...
  constructor (client, initOpts) {
    ...
    /** @type {StorageManager} */
    this._storageManager = new StorageManager(client)
    ...
    client.on('Page.fileChooserOpened', this._onFileChooser.bind(this))
  ...
  /**
   * @return {Chrome|CRIConnection|CDPSession|Object}
   */
  get connection () {
    return this._client
  }
  ...
  /**
   * Returns a ElementHandle for the main frames document object
  ...
  /**
   * Returns a JSHandle for the main frames window object
   * @return {Promise<JSHandle>}
   */
  window () {
    return this.mainFrame().window()
  }
  ...
  /**
   *
   * @param {{depth?: number, pierce?: boolean, path?: string}} [options]
   * @return {Promise<Object>}
   */
  async getJSONRepresentationOfDom (options) {
    const { depth = -1, pierce = true, path = null } = options || {}
    const results = await this._client.send('DOM.getDocument', {
      depth,
      pierce
    })
    if (path) {
      await fs.writeJson(path, results.root)
    }
    return results.root
  }
   ...
   */
  async getFlattenedJSONRepresentationOfDom (options) {
    const { depth = -1, pierce = true, path = null } = options || {}
    const results = await this._client.send('DOM.getFlattenedDocument', {
      depth,
      pierce
    })
    if (path) {
      await fs.writeJson(path, results.nodes)
    }
