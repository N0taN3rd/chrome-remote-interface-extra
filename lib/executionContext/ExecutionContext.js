...
class ExecutionContext {
  ...
  evaluateHandle (pageFunction, ...args) {
    return this._evaluateHandleImpl(pageFunction, args, false)
  }
  ...
  evaluateHandleWithCliAPI (pageFunction, ...args) {
    return this._evaluateHandleImpl(pageFunction, args, true)
  }
  ...
  async evaluate (pageFunction, ...args) {
    const handle = await this._evaluateHandleImpl(pageFunction, args, false)
  ...
  async evaluateWithCliAPI (pageFunction, ...args) {
    const handle = await this._evaluateHandleImpl(pageFunction, args, true)
    const result = await handle.jsonValue().catch(error => {
      if (error.message.includes('Object reference chain is too long')) return
      if (error.message.includes("Object couldn't be returned by value")) return
      throw error
  ...
  globalObject () {
    ...
    return this.evaluateHandle(() => self)
  }
   ...
   * @return {Promise<JSHandle>}
   */
  async _evaluateHandleImpl (pageFunction, args, withCliAPI) {
