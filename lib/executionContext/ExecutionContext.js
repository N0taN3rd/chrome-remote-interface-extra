...
class ExecutionContext {
  ...
  evaluateHandle (pageFunction, ...args) {
    return this._evaluateHandleImpl(false, false, pageFunction, args)
  }
  ...
  evaluateHandleWithCliAPI (pageFunction, ...args) {
    return this._evaluateHandleImpl(false, true, pageFunction, args)
  }
  ...
  evaluate (pageFunction, ...args) {
    return this._evaluateHandleImpl(true, false, pageFunction, args)
  }
  ...
  evaluateWithCliAPI (pageFunction, ...args) {
    return this._evaluateHandleImpl(true, true, pageFunction, args)
  }
  ...
  globalObject () {
    ...
    return this._evaluateHandleImpl(false, false, () => self)
  }
   ...
   * @param {boolean} withCliAPI
   * @param {Function|string} pageFunction
   * @param {Array<*>} [args]
   * @return {Promise<*>}
   */
  async _evaluateHandleImpl (returnByValue, withCliAPI, pageFunction, args) {
    if (helper.isString(pageFunction)) {
      ...
      const { exceptionDetails, result } = await this._client
        .send('Runtime.evaluate', opts)
        .catch(rewriteError)
      if (exceptionDetails) {
        throw new Error(
      ...
      return returnByValue
        ? helper.valueFromRemoteObject(result)
        : createJSHandle(this, result)
    }
    ...
    const { exceptionDetails, result } = await callFunctionOnPromise.catch(
      rewriteError
    )
    if (exceptionDetails) {
      throw new Error(
