...
class Frame {
   ...
   */
  constructor (frameManager, cdpClient, parentFrame, frameId) {
  ...
  _addExecutionContext (context) {
    if (context._isDefault) this._setDefaultContext(context)
  }
  ...
  _removeExecutionContext (context) {
    if (context._isDefault) this._setDefaultContext(null)
  }
  ...
  async evaluateExpression (expression, withCliApi = false) {
    const context = await this._contextPromise
    if (context == null) {
      throw new Error(
        `Frame id = ${this._id} does not have an execution context`
      )
    }
    return context.evaluateExpression(expression, withCliApi)
  }
  ...
  waitForFunction (pageFunction, options = {}, ...args) {
    const { polling = 'raf', timeout = 30000 } = options
    return new WaitTask(
      this,
      pageFunction,
      'function',
