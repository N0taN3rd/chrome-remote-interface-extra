...
class NetworkManager extends EventEmitter {
  ...
  async deleteCookies (cookie) {
    await this._client.send('Network.clearBrowserCookies', cookie)
  }
...
class Cookie {
  ...
  async deleteCookie () {
    const deleteMe = { name: this._name }
    if (this._path) {
      deleteMe.path = this._path
    } else if (this._domain) {
      deleteMe.domain = this._domain
    }
