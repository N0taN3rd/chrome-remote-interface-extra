...
class Response {
  ...
  ok () {
    const test =
      this._status === 0 || (this._status >= 200 && this._status <= 299)
    if (!test) return this._status === 304
    return test
  }
  ...
  requestInterceptionId () {
    ...
    return req ? req.requestInterceptionId() : null
  }
   ...
   */
  bodyLoadedPromise () {
    return this._bodyLoadedPromise
  }
