...
const { headersArray, StatusToMessage } = require('./_shared')
...
class Fetch extends EventEmitter {
  ...
  async respond (opts) {
    ...
    let hasContentLengthHeader = false
    const cntLenCases = new Set(['content-length', 'Content-Length'])
    if (opts.headers) {
      for (const header of Object.keys(opts.headers)) {
        if (!hasContentLengthHeader && cntLenCases.has(header)) {
          hasContentLengthHeader = true
        }
        responseHeaders[header] = opts.headers[header]
      }
    }
    if (opts.contentType) {
      responseHeaders['Content-Type'] = opts.contentType
    }
    if (responseBody && !hasContentLengthHeader) {
    ...
    await this.fulfillRequest({
      ...
      responsePhrase: StatusToMessage[opts.status || 200],
