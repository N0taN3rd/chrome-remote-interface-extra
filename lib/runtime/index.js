...
const EVALUATION_SCRIPT_URL = '__cri-extra_evaluation_script__'
...
class ExecutionContext {
  ...
  constructor (client, contextPayload, frame) {
    ...
    this._isDefault = contextPayload.auxData
...
class ElementHandle extends JSHandle {
   ...
   */
  constructor (context, client, remoteObject, frameManager) {
  ...
  mouse () {
    if (this._mouse == null) {
      this._mouse = new Mouse(this._client, this.keyboard())
    }
    return this._mouse
  }
  ...
  touchscreen () {
    if (this._touchScreen == null) {
      this._touchScreen = new Touchscreen(this._client, this.keyboard())
    }
  ...
  async click (options) {
    await this._scrollIntoViewIfNeeded()
    const { x, y } = await this._clickablePoint()
    await this.mouse().click(x, y, options)
  }
  ...
  async tap () {
    await this._scrollIntoViewIfNeeded()
    const { x, y } = await this._clickablePoint()
    await this.touchscreen().tap(x, y)
  }
  ...
  async press (key, options) {
    await this.focus()
    await await this.keyboard().press(key, options)
  }
...
/**
 * @type {{JSHandle: JSHandle, ExecutionContext: ExecutionContext, ElementHandle: ElementHandle, createJSHandle: createJSHandle, EVALUATION_SCRIPT_URL: string}}
 */
module.exports = {
  ExecutionContext,
  JSHandle,
